...
bucket_name = ssm.get_parameter(Name="dragon_data_bucket_name", WithDecryption=False)[
    "Parameter"
]["Value"]
file_name = ssm.get_parameter(Name="dragon_data_file_name", WithDecryption=False)[
    "Parameter"
]["Value"]
...
def listDragons(event, context):
    ...
    if "queryStringParameters" in event and event["queryStringParameters"] is not None:
        if "dragonName" in event["queryStringParameters"]:
            expression = (
                "select * from S3Object[*][*] s where s.dragon_name_str =  '"
                + event["queryStringParameters"]["dragonName"]
                + "'"
            )
